...
def makeAFigure():
	...
	nhtsa_line = plt.Rectangle((1975, y_nhtsa-lineH/2), (x_merge-1975-0.5), lineH, color = nhtsa, linewidth = 0, alpha = 1)
	nhtsa_connector = plt.Rectangle((x_merge-lineH/2, y_nhtsa+lineH/2), lineH, y_merged-y_nhtsa-2*lineH, color = nhtsa, linewidth=0)
	mergednhtsa_line = plt.Rectangle((x_merge+0.5, y_merged-3*lineH/2), (2026.5-x_merge-0.5), lineH, color = nhtsa, linewidth = 0, alpha = 1)
	...
	def addNHTSADates():
		...
		plt.plot((1973.5, 1973.5), (y_nhtsa+tickH, y_nhtsa+5*tickH), '-k')
		plt.plot((1974,1974),(y_nhtsa, y_nhtsa+tickH), '-k')
		plt.plot((1973, 1974), (y_nhtsa+tickH, y_nhtsa+tickH), '-k')
		plt.text(1973.5, y_nhtsa+5.25*tickH, '1973-74: Oil Embargo', fontsize = fs, horizontalalignment = 'left', verticalalignment = 'bottom')#, rotation = 90)

		plt.plot((1975, 1975), (y_nhtsa, y_nhtsa+4*tickH), '-k')
		plt.plot(1975, y_nhtsa, '.k')
		plt.text(1975-0.5, y_nhtsa+4.25*tickH, '1975: EPCA Passed', fontsize = fs, horizontalalignment = 'left', verticalalignment = 'bottom')#, rotation = 90)

		plt.plot((1978, 1978), (y_nhtsa, y_nhtsa+3*tickH), '-k')
		plt.plot(1978, y_nhtsa, '.k')
		plt.text(1977, y_nhtsa+3.25*tickH, '1978: Passenger Car Standards begin at ' + str(car.CarStd.iloc[0]) + ' mpg', fontsize = fs)
	...
	prepa_line = plt.Rectangle((2005, y_epa-lineH/2), 2009-2005, lineH, color = epa, linewidth = 0, alpha = 0.5)
	#epa_line = plt.Rectangle((2009, y_epa-lineH/2), 2, lineH, color = epa, linewidth = 0, alpha = 1)
	mergedepa_line = plt.Rectangle((x_merge, y_merged-lineH/2), (2026.5-x_merge), lineH, color = epa, linewidth = 0, alpha = 1)
	...
	caW1 = mpatches.Wedge((x_merge-lineH/2, y_ca-lineH/2), lineH, 0, 90, color = ca, linewidth = 0)
	caW2 = mpatches.Wedge((x_merge+lineH/2, y_merged + 3*lineH/2), lineH, 180, 270, color = ca, linewidth = 0)
	ca_connector = plt.Rectangle((x_merge-lineH/2, y_merged+3*lineH/2), lineH, y_ca-y_merged-2*lineH, color = ca, linewidth=0)
	...
	def addMergedDates():
		...
		plt.plot((2012, 2012), (y_merged, y_merged-5*tickH), '-k')
		...
		plt.text(2011.5, y_merged-5.25*tickH, '2012: New standards for 2017-2025,\n54.5 mpg overall by 2025', fontsize = fs, horizontalalignment = 'left', verticalalignment = 'top')	

		plt.plot(2018.67, y_merged, '.k')
		plt.plot((2018.67, 2018.67), (y_merged, y_merged+9*lineH), '-k')
		...
		plt.text(2018, y_merged + 9.25*lineH, 'August 2018: Trump\nAdmin. Proposed\nRollback', fontsize = fs, horizontalalignment = 'left', verticalalignment = 'bottom')


		plt.plot((2009, 2009), (-7, 12), '-k')
		...
		plt.text(2008.5, 12.25, '2009: Obama Admin. National Program', fontsize = fs, horizontalalignment = 'left', verticalalignment = 'bottom')#, rotation = 90)
	addMergedDates()
	...
	def addMPGBars():
		...
		for n in range(len(carAll.Year)):
			if n<len(carAll.Year)-1:
				addMPGdata(carAll.Year.iloc[n], carAll.CarStd.iloc[n]/5, carAll.CarStd.iloc[n+1]/5, y_merged+3*lineH/2, 1, 'car', 0)
			else:
				addMPGdata(carAll.Year.iloc[n], carAll.CarStd.iloc[n]/5, 0, y_merged+3*lineH/2, 1, 'car', 1)
			#ax1.add_patch(r)

		for n in range(len(carObama.Year)):
			if n<len(carObama.Year)-1:
				addMPGdata(carObama.Year.iloc[n], carObama.CarStd.iloc[n]/5, carObama.CarStd.iloc[n+1]/5, y_merged+3*lineH/2, 0.3, 'car', 0)
			else:
				addMPGdata(carObama.Year.iloc[n], carObama.CarStd.iloc[n]/5, 0, y_merged+3*lineH/2, 0.3, 'car', 1)
			#ax1.add_patch(r)
		plt.text(carObama.Year.iloc[-3]+0.25, carObama.CarStd.iloc[-3]/5+y_merged+lineH, 'Obama Rule', fontsize = fs-2, color = carcolor)
		...
		#	else:
			addMPGdata(carTrump.Year.iloc[n], carTrump.CarStd.iloc[n]/5, 0, y_merged+3*lineH/2, 0.4, 'car', 1)
			#ax1.add_patch(r)
		plt.text(carTrump.Year.iloc[-3]+0.25, carTrump.CarStd.iloc[-3]/5+y_merged+lineH, 'Trump Proposal', fontsize = fs-2, color = carcolor)

		plt.plot((carObama.Year.iloc[0], carObama.Year.iloc[0]), (carObama.CarStd.iloc[0]/5+y_merged+3*lineH/2, carAll.CarStd.iloc[-1]/5+y_merged+3*lineH/2), color = carcolor, alpha = 0.3)
		...
		for n in range(len(truckAll.Year)):
			if n <len(truckAll.Year)-1:
				addMPGdata(truckAll.Year.iloc[n], truckAll.LTStd.iloc[n]/5, truckAll.LTStd.iloc[n+1]/5, y_merged+3*lineH/2, 1, 'truck', 0)
			else:
				addMPGdata(truckAll.Year.iloc[n], truckAll.LTStd.iloc[n]/5, 1, y_merged+3*lineH/2, 1, 'truck', 1)
			#ax1.add_patch(r)

		for n in range(len(truckObama.Year)):
			if n <len(truckObama.Year)-1:
				addMPGdata(truckObama.Year.iloc[n], truckObama.LTStd.iloc[n]/5, truckObama.LTStd.iloc[n+1]/5, y_merged+3*lineH/2, 0.3, 'truck',0)
			else:
				addMPGdata(truckObama.Year.iloc[n], truckObama.LTStd.iloc[n]/5, 0, y_merged+3*lineH/2, 0.3, 'truck',1)
			#ax1.add_patch(r)
		...
		for n in range(len(truckTrump.Year)):
			addMPGdata(truckTrump.Year.iloc[n], truckTrump.LTStd.iloc[n]/5, 0, y_merged+3*lineH/2, 0.4, 'truck',1)
		plt.text(truckTrump.Year.iloc[-3]+0.25, truckTrump.LTStd.iloc[-3]/5+y_merged+lineH, 'Trump Proposal', fontsize = fs-2, color = truckcolor)
			#ax1.add_patch(r)
	addMPGBars()
	...
	plt.savefig('TimelineFigure.png', dpi = 300)
	plt.clf()
